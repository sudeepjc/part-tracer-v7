...
const CONNECTION_PROFILE_PATH = path.join(__dirname ,'../partTracer-airbus.yaml');
const CRYPTO_CONFIG_PEER_ORGANIZATIONS =path.join(__dirname ,'../../organizations/peerOrganizations');
const AIRBUS_CLIENT_CONNECTION_PROFILE_PATH = path.join(__dirname,'../airbusOrgClient.yaml');
...
async function getClientContext(user,org){
  ...
  client.loadFromConfig(AIRBUS_CLIENT_CONNECTION_PROFILE_PATH);
...
function getCertPath(org, user) {
    ...
    var certPath = CRYPTO_CONFIG_PEER_ORGANIZATIONS + "/" + org + ".parttracer.com/users/" + user + "@" + org + ".parttracer.com/msp/signcerts/cert.pem"
    return certPath
}
...
function getPrivateKeyPath(org, user) {
    ...
    var pkFolder = CRYPTO_CONFIG_PEER_ORGANIZATIONS + "/" + org + ".parttracer.com/users/" + user + "@" + org + ".parttracer.com/msp/keystore"
