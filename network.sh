...
# "organizations/regulatorOrganizations" directory.

# Create Organziation crypto material using cryptogen or CAs
function createOrgs() {

  if [ -d "organizations/peerOrganizations" ]; then
    rm -Rf organizations/peerOrganizations && rm -Rf organizations/regulatorOrganizations
  fi
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create general Identities ######################"
    echo "##########################################################"
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    echo "##########################################################"
    echo "############ Create General Org Identities ######################"
    echo "##########################################################"

    createGeneral

    echo "##########################################################"
    echo "############ Create Airbus Org Identities ######################"
    echo "##########################################################"

    createAirbus

    echo "##########################################################"
    echo "############ Create Regulator Org Identities ###############"
    echo "##########################################################"
  ...
  echo
  echo "Generate CCP files for general and airbus"
...
# file for creating the network is "docker-compose-part-tracer.yaml" in the ``docker``
...
## call the script to join create the channel and join the peers of general and airbus
...
function networkDown() {
  # stop org3 containers also in addition to general and airbus, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf system-genesis-block/*.block organizations/peerOrganizations organizations/regulatorOrganizations
    ## remove fabric ca artifacts
    rm -rf organizations/fabric-ca/general/msp organizations/fabric-ca/general/tls-cert.pem organizations/fabric-ca/general/ca-cert.pem organizations/fabric-ca/general/IssuerPublicKey organizations/fabric-ca/general/IssuerRevocationPublicKey organizations/fabric-ca/general/fabric-ca-server.db
    ...
    rm -rf channel-artifacts log.txt partTracer.tar.gz system-genesis-block

    # remove both the org wallets
    rm -rf generalOrgApp/identity airbusOrgApp/identity
    rm -rf generalOrgApp/credstore airbusOrgApp/credstore
...
COMPOSE_FILE_BASE=docker/docker-compose-part-tracer.yaml
